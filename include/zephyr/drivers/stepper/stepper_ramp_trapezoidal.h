...
struct stepper_ramp_trapezoidal_data
{
	...
	/**
	 * Represents the number of steps remaining during the pre-deceleration phase of a motor's ramp profile.
	 *
	 * This variable is used in the motor control logic to monitor the progress of the motor while
	 * speeding down. It helps ensure precise timing and smooth transitions as the motor decreases
	 * its speed to a lower velocity, contributing to accurate and efficient motion control.
	 */
	uint32_t pre_decel_steps_left;

	/**
	 * Represents the number of steps remaining during the acceleration phase of a motor's ramp profile.
	 *
	 * This variable is used in the motor control logic to monitor the progress of the motor while
	 * speeding up. It helps ensure precise timing and smooth transitions as the motor increases
	 * its speed, contributing to accurate and efficient motion control.
	 */
	uint32_t accel_steps_left;

	/**
	 * Represents the number of steps remaining to be executed in the current phase of motor control.
	 *
	 * This variable is a critical part in the motion control algorithm, used to monitor the progress
	 * of a motor in its ramp profile. It is decreased progressively as steps are executed, ensuring precise
	 * tracking of motor movement during acceleration, constant-speed, or deceleration phases. This contributes
	 * to achieving smooth, efficient, and accurate motor operations.
	 */
	uint32_t run_steps_left;

	/**
	 * Represents the number of steps remaining during the deceleration phase of a motor's ramp profile.
	 *
	 * This variable is used in motor control algorithms to track the progress of deceleration, ensuring
	 * smooth and accurate transitions as the motor slows down. It is progressively decreased as steps
	 * are executed, contributing to precise motion control and efficient deceleration management.
	 */
	...
	/**
	 * Indicates the current index in the acceleration or deceleration sequence of a motor's ramp profile.
	 *
	 * This variable is used to track the progression of calculation steps during both the acceleration
	 * and deceleration phases of the ramping logic. It plays a vital role in determining the adjustments
	 * needed for step intervals, ensuring accurate and smooth transitions in motor motion. Incremented
	 * or decremented depending on the operational phase, it is integral to precise motion control.
	 */
	uint32_t acceleration_idx;

	/**
	 * Represents the current time interval, in nanoseconds, for a motor step during
	 * its operational ramp profile, including acceleration, steady speed, or deceleration phases.
	 *
...
#define STEPPER_RAMP_TRAPEZOIDAL_DEFINE(name, acceleration_rate_p, run_interval_p, deceleration_rate_p) \
