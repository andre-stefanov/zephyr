...
static int stepper_motion_controller_perform_step(const struct device *dev)
{
	...
	const int ret = controller->config->callbacks->step(dev);
                                      ...
                                      const int32_t micro_steps)
{
	...
	K_SPINLOCK(&data->lock) {
		...
		const bool is_moving = config->timing_source_api->get_interval(
			                       config->timing_source) > 0;
		...
		if (is_moving && !is_moving_in_same_direction) {
			...
			movement_steps_count = stepper_ramp_prepare_move(
				data->ramp, abs(micro_steps));
		}
