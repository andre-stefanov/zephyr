...
STEPPER_RAMP_CONSTANT_DEFINE(ramp_constant, 500000);

STEPPER_RAMP_TRAPEZOIDAL_DEFINE(ramp_trapezoidal, 10000, 1000000, 1000);

static struct stepper_ramp_base *current_ramp = ramp_trapezoidal;
...
static enum stepper_mode current_mode = STEPPER_MODE_ENABLE;
...
int main(void)
{
	...
	// stepper_set_ramp(stepper, ramp_constant);
	stepper_set_ramp(stepper, ramp_trapezoidal);
